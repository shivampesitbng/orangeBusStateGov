...
export default {
  ...
  methods : {
    ...
    get_distance_between_stops(bus){
      ...
      console.log("get_distance_between_stops_function -> "+bus.no + " "
        + bus.route_name+ " "
        + bus.route);
      this.$http.get('bus_travel_time_in_between_stops/'+
        + str1 + '.json')
        .then(response => {
        ...
        .then(dist => {
          ...
          if(str2 == 'Y'){
            ...
            this.dist_in_between_stops_arr.push("0 min");
            let dist_sum = 0;
            for(let i=marked_src ; i<=marked_dest-1 ; i++){
              console.log("dist->"+dist[i]);
              dist_sum = dist_sum + dist[i];
              this.dist_in_between_stops_arr.push(dist_sum+ " min");
            }

            //
            let time_sum_frm_origin_to_src = 0;
            for(let i=0 ; i<marked_src ; i++){
               console.log("origin to src - loop cnt");
               time_sum_frm_origin_to_src = time_sum_frm_origin_to_src + dist[i];
            }
            console.log("min from origin to src -> "+time_sum_frm_origin_to_src);
            this.$store.state.time_sum_frm_origin_to_src =
              time_sum_frm_origin_to_src;
            //
            ...
            for(let i=bus.route.length-1 ; i>=0 ; i--){
              console.log("bus route->"+bus.route[i]);
              if(this.$store.state.selected_source.name == bus.route[i]){
                marked_src = i ;
                console.log("maraked source->"+this.$store.state.selected_source.name+" "+
                  + marked_src) ;
              }
              if(this.$store.state.selected_destination.name == bus.route[i]){
                marked_dest = i ;
            ...
            //
            let time_sum_frm_origin_to_src = 0;
            console.log("route_len->"+(bus.route.length-1) + " | marked_src-> " + marked_src);
            for(let i=bus.route.length-2 ; i>=marked_src ; i--){
               console.log("origin to src - loop cnt | test-> " + dist[i] );
               time_sum_frm_origin_to_src = time_sum_frm_origin_to_src + dist[i];
            }
            console.log("min from origin to src -> "+time_sum_frm_origin_to_src);
            this.$store.state.time_sum_frm_origin_to_src =
  ...
  computed :{
    ...mapGetters([
      'selected_bus_detail','time_sum_frm_origin_to_src'
    ])
  },
  beforeMount()  {
